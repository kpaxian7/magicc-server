...
def get_session_stable_cpu(model_path: str):
    so = ort.SessionOptions()
    so.log_severity_level = 3
    # ⛔️ 禁用所有图优化，跳过 SimplifiedLayerNormFusion
    so.graph_optimization_level = ort.GraphOptimizationLevel.ORT_DISABLE_ALL
    n = max(2, multiprocessing.cpu_count())
    so.intra_op_num_threads = n
    so.inter_op_num_threads = max(1, n // 2)

    providers = ["CPUExecutionProvider"]
    print(f"[INFO] providers={providers} (ORT_DISABLE_ALL)")
    return ort.InferenceSession(model_path, sess_options=so, providers=providers)

def get_session(model_path: str,
                prefer_gpu: bool = False,  # Ubuntu+NVIDIA: True
                prefer_coreml: bool = True,  # macOS: True（如果之前能跑通）
                force_cpu: bool = False):  # 兜底 CPU
    so = ort.SessionOptions()
    # ====== 性能优先的设置 ======
    # 开启最高级图优化
    so.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
    # 线程尽量给满（你也可以手动写个具体数字，如 8/16）
    n_threads = max(2, multiprocessing.cpu_count())
    so.intra_op_num_threads = n_threads
    so.inter_op_num_threads = max(1, n_threads // 2)
    so.execution_mode = ort.ExecutionMode.ORT_PARALLEL
    # 开启内存模式，提高吞吐
    so.enable_mem_pattern = True
    so.enable_cpu_mem_arena = True
    # 降低日志噪音（但保留 WARNING，观察问题）
    os.environ["ORT_LOG_SEVERITY_LEVEL"] = "2"  # 2=WARNING

    providers = []
    if prefer_gpu:
        # 需要: pip install onnxruntime-gpu
        providers = ["CUDAExecutionProvider", "CPUExecutionProvider"]
    elif prefer_coreml:
        # macOS 上使用 CoreML（你之前能出结果，只是有 warning，可以忽略）
        # 注意：某些机型/ORT版本仍可能刷警告，但一般不影响正确性和速度
        providers = ["CoreMLExecutionProvider", "CPUExecutionProvider"]
    if force_cpu or not providers:
        providers = ["CPUExecutionProvider"]

    print(f"[INFO] FAST providers: {providers}, threads={n_threads}")
    return ort.InferenceSession(model_path, sess_options=so, providers=providers)
...
def main():
    ...
    # session = get_session(MODEL_PATH, prefer_coreml=True, prefer_gpu=False, force_cpu=False)
    session = get_session_stable_cpu(MODEL_PATH)
    ...
    # 打印性能统计
    print(f"\n{'='*50}")
    print(f"性能统计 - test_rmbg_20_fp16.py")
    print(f"{'='*50}")
